...
int cmppid(const char*, const char*);
...
void printprompt(void)
{
	char* wd=(char*)malloc(PATH_MAX*sizeof(char));
	getcwd(wd, PATH_MAX);
...
void execute_cd(char* arg)
{
	...
	if(chdir(arg)!=0)
	{
		char* errmsg=(char*)malloc((5+PATH_MAX)*sizeof(char));
		sprintf(errmsg, "%s: %s", cd_cmd, arg);
		perror(errmsg);
		free(errmsg);
		free(wd);
		return;
	...
	getcwd(wd, PATH_MAX);
	setenv(pwdvar, (const char*)wd, 1);
}

int main(int argc, char** argv, char** envp)
{
	size_t s, t, npipes;
	char* tknd, * in, * cmd, * path;
	char** split, ** paths;
	char*** pipesplit;
	int ppfd[2];
	pid_t p;
	...
	t=0;
	tknd=strtok(path+strlen(pathvar), ":");
	paths[t++]=tknd;
	...
	while(!feof(stdin))
	{
		...
		if(strcmp(pipesplit[0][0], cd_cmd)==0)
		{
			if(pipesplit[0][1]!=NULL&&pipesplit[0][2]!=NULL)
			{
				printf("Usage: cd <dir>\n");
				goto noexec;
			}
			execute_cd(pipesplit[0][1]);
			goto noexec;
		}
